<template>
  <div class='search-block'>
    ...
    <input placeholder='Search HCP/HCO' :value='selected' @input='handleInput' ref='inputSearch'>
    <div v-show='validHPCs.length'>
      <p v-for='(item, index) in validHPCs' :key='index' @click='handleClick(item)'>{{ item }}</p>
    </div>
  </div>
</template>
...
const handleInput = () => {
  ...
  timer = setTimeout(() => {
    nextTick(() => {
      if (props.HCPs) {
        validHPCs.value = props.HCPs.filter(p => p.indexOf(inputSearch.value.value) > -1)
