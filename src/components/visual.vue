<template>
    <div class='visual-block'>
        ...
        <div class='line-3'>
            <span>
                <svg-icon name='calculate' color='lightgreen' background='transparent' width='18px'
...
const calculate = (scheduleList: schedule[]) => {
    scheduleList.forEach(p => {
        Object.keys(typeObj).forEach((key: string) => {
            if (p.category === key) {
                if ((props.periodStart <= new Date(p.scheduleTime).getTime()) && (new Date(p.scheduleTime).getTime() <= props.periodEnd)) {
                ...
                if ((props.periodStart <= new Date(p.confirmedTime).getTime()) && (new Date(p.confirmedTime).getTime() <= props.periodEnd)) {
                    report.value.target[key as keyof typeof typeObj] += 1;
