<template>
  <div class='schedule-block'>
    <div class='schedule-unit' v-for=' in scheduleUnits' :style="{ width: scheduleUnit_width }">
    </div>
    <svg-icon class='schedule_svg' v-for='(scheduleItem, index) in scheduleList' :key='index' :name='scheduleItem.name'
...
const props = defineProps(['data', 'periodStart', 'periodEnd', 'scheduleUnits', 'scheduleUnit_width'])

/**
 * @description setup object to display svg
 */
type item_type = { id: number, name: string, isDone: boolean, leftRate: string, color: string }
...
const initData = (list: schedule[]) => {
  ...
  list.forEach((item: schedule, index: number) => {
    const newScheduleObj: item_type = { id: index, name: item.category, isDone: false, leftRate: '', color: 'lightgreen' }
    const newConfirmObj: item_type = { id: index, name: item.category, isDone: false, leftRate: '', color: '' }
    const scheduleTimestamp = new Date(item.scheduleTime).getTime()
    const confirmTimestamp = new Date(item.confirmedTime).getTime()
    if ((props.periodStart <= scheduleTimestamp && scheduleTimestamp <= props.periodEnd)) {
      const rate: string = ((scheduleTimestamp - props.periodStart) / totalWide).toFixed(2)
      newScheduleObj.leftRate = `${parseFloat(rate) * 100}%`
      scheduleList.value.push(newScheduleObj)
    }
    if ((props.periodStart <= confirmTimestamp && confirmTimestamp <= props.periodEnd)) {
      newConfirmObj.isDone = true
      const rate: string = ((confirmTimestamp - props.periodStart) / totalWide).toFixed(2)
