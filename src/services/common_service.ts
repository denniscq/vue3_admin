...
export const calculatePeriodForWeek = (inputDate?: number): periodImp => {
    const result: periodImp = { periodStart: 0, periodEnd: 0, scheduleUnits: [1, 2, 3, 4, 5, 6, 7], displayValue: '' }
    ...
    if (date >= dayInWeek) {
        ...
        result.periodStart = lastMonthEndtime - (dayInWeek - date) * ONEDAY_TIMESTAMP
    }
...
export const calculatePeriodForMonth = (inputDate?: number): periodImp => {
    const result: periodImp = { periodStart: 0, periodEnd: 0, scheduleUnits: [], displayValue: '' }
...
export const switchWeekOrMonth = (modelFlag: string, callback: Function) => {
    const result = modelFlag === '1' ? calculatePeriodForWeek() : calculatePeriodForMonth()
